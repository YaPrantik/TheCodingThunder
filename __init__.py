...
app.config.update(
    ...
    MAIL_PORT='465',
    MAIL_USE_SSL=True,
    MAIL_USERNAME=params['gmail_sender_user'],
    MAIL_PASSWORD=params['gamil_sender_password'])
...
class About(db.Model):
    ...
    name = db.Column(db.String(80), nullable=False)
    slug = db.Column(db.String(50), nullable=False)
    about_text = db.Column(db.String(80), nullable=False)
    about_tline = db.Column(db.String(80), nullable=False)
        ...
        if request.method == 'POST':
            ...
            if sno == '0':
                post = Posts(title=box_title, slug=box_slug, content=box_content,
                             tline=box_tline, img_file=box_img_file, date=date)
                db.session.add(post)
                db.session.commit()
            else:
...
@app.route("/about/<string:about_slug>")
def about_route(about_slug):
    about = About.query.filter_by(slug=about_slug).first()
...
def contact():
    if (request.method == 'POST'):
        ...
        mail.send_message('New message from ' + name,
                          sender=email,
                          recipients=[params['gmail_rcvr_user']],
                          body="Name: " + name + "\n" + "Phone_Num: "+phone + "\n" + "Message: "+message)
